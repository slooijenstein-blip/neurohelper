...
Auth is Clerk (`@clerk/react` SPA SDK). Schedule / community / journey data still lives in the browser (`localStorage` key `motor-skill-buddy-v1`). There is no app database yet. Child first name/age are device-local and are not stored on the Clerk user.
...
src/
  ...
  components/app/   # Product UI: tabs, auth, community, schedule
