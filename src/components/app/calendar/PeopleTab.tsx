      ...
      <div className="hide-scrollbar min-h-0 flex-1 space-y-3 overflow-y-auto bg-surface px-5 py-4 md:max-w-2xl md:px-8">
        ...
        {people.map((person) => (
          ...
          >
            ...
            {(canChangeRole(role) || canRemovePerson(role)) &&
            person.role !== "therapist" ? (
              <DropdownMenu>
          ...
          <div className="space-y-3">
            ...
            {lastLink ? (
              ...
              <Button
                ...
                className="w-full"
                onClick={() => {
                  const invite = cal.createInvite(child.id, email || "demo@example.com", inviteRole, displayName || "Guest");
                  if (!invite) return;
                  const link = `${window.location.origin}${withBasePath(`/invite/${invite.token}`)}`;
                  setLastLink(link);
                  void copyLink(link);
