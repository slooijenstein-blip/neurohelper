...
export function TodayTab({ onOpenLibrary }: { onOpenLibrary: () => void }) {
  ...
  const onPickStep = (step: PlanStep) => {
    if (replacingId) {
      ...
      toast.success(t("calendar.today.stepAdded", { title: presentPlanStep(step, locale).title }));
    }
  };
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
        ...
        {!plan || plan.steps.length === 0 ? (
          ...
          <div className="space-y-2">
            <div className="flex items-baseline justify-between gap-2">
              <h3 className="font-display text-base font-semibold">{presentPlanName(plan, t)}</h3>
      ...
      <ActivityPickerDialog
        ...
        onPick={onPickStep}
        title={replacingId ? t("calendar.picker.changeTitle") : t("calendar.picker.addToToday")}
      />
