...
export function ScheduleTab() {
  ...
  const toggleDone = (id: string) =>
    update((prev) => ({
      ...
      completedCount: prev.completedCount + (state.schedule.find((i) => i.id === id)?.done ? -1 : 1),
    }));
  ...
  const saveTemplate = () => {
    ...
    update((prev) => ({ ...prev, templates: [t, ...prev.templates] }));
    setTemplateName("");
    setRepeatDays([]);
    setSaveOpen(false);
    toast.success(
  ...
  const addToGoogleCalendar = () => {
    ...
    const url =
      ...
      `&text=${encodeURIComponent("NeuroHelper routine")}` +
    ...
    <div className="flex h-full min-h-0 flex-col">
        ...
        right={
          state.schedule.length ? (
            <div className="flex gap-1">
              <Button size="icon" variant="ghost" className="size-8" onClick={resetSchedule} title="Reset">
                <RotateCcw className="size-4" />
              </Button>
              <Button size="icon" variant="ghost" className="size-8" onClick={() => setSaveOpen(true)} title="Save">
                <Save className="size-4" />
              </Button>
              <Button size="icon" variant="ghost" className="size-8" onClick={() => setShareOpen(true)} title="Share">
      ...
      <div className="hide-scrollbar min-h-0 flex-1 space-y-3 overflow-y-auto bg-surface px-5 py-4 md:max-w-3xl md:px-8">
        <div className="flex gap-1 rounded-xl bg-card p-1">
          {(["list", "calendar"] as const).map((v) => (
            ...
            >
              {v === "list" ? <ListChecks className="size-3.5" /> : <CalendarDays className="size-3.5" />}
              {v === "list" ? "Today" : "Calendar"}
            </button>
          ))}
        </div>
        ...
        {state.schedule.length === 0 ? (
          <div className="soft-card py-8 text-center">
            ...
            <Button className="mt-3" onClick={() => setAdding(true)}>
              <Plus className="mr-1 size-4" /> Add an activity
            </Button>
          </div>
        ) : (
          <div className="space-y-2">
            {state.schedule.map((item) => (
              <div key={item.id} className="soft-card flex items-center gap-3 p-3">
                ...
                <div className="min-w-0 flex-1">
                  <p className={cn("truncate text-sm font-semibold", item.done && "text-muted-foreground line-through")}>
            ...
            <Button variant="outline" className="w-full" onClick={() => setAdding(true)}>
              <Plus className="mr-1 size-4" /> Add another activity
            </Button>
          </div>
        )}
        ...
        {state.templates.some((t) => t.ownerId === state.profile?.id && !t.isPublic) ? (
          <div className="soft-card p-4">
            <h3 className="mb-2 text-sm font-semibold">Your templates</h3>
            <div className="space-y-2">
              {state.templates
                .filter((t) => t.ownerId === state.profile?.id && !t.isPublic)
                .map((t) => (
                  <div key={t.id} className="flex items-center justify-between rounded-lg border border-border bg-card p-2">
                    ...
                    <Button
                      ...
                      onClick={() =>
                        update((prev) => ({
                          ...prev,
                          schedule: t.items.map((i) => ({ ...i, done: false })),
                        }))
                      }
                    >
                      Load
      ...
      <Dialog open={adding} onOpenChange={setAdding}>
        <DialogContent className="max-w-sm">
          ...
          <Input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search activities..." />
            ...
            {filtered.map((a: Activity) => (
              ...
              >
                ...
                <p className="text-[11px] text-muted-foreground">
                  {a.minAge}-{a.maxAge} yrs · {a.minMinutes}-{a.maxMinutes} mins · {a.skill}
                </p>
              </button>
            ))}
            {!filtered.length ? <p className="py-2 text-center text-xs text-muted-foreground">No matches.</p> : null}
          </div>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={shareOpen} onOpenChange={setShareOpen}>
        <DialogContent className="max-w-sm">
          ...
          <textarea
            ...
            placeholder="Say something about this routine..."
            rows={3}
            className="w-full rounded-md border border-input bg-card px-3 py-2 text-sm outline-none"
          />
          <Button onClick={shareTemplate}>Share to community</Button>
        </DialogContent>
      </Dialog>
    </div>
  );
