...
export function CalendarStoreProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (myChildren.length === 1 && state.selectedChildId !== myChildren[0].id) {
      update((prev) => ({ ...prev, selectedChildId: myChildren[0].id }));
    }
  ...
  const value: CalendarContextValue = {
    ...
    addChild: (displayName, ageBand, tagIds = []) => {
      ...
      update((prev) => ({
        ...
        childTags: [
          ...prev.childTags,
          ...tagIds.map((tagId) => ({ childId: child.id, tagId })),
        ],
    ...
    visibleLibrary: (childId) => {
      ...
      return state.libraryPlans.filter(
        (p) =>
          p.ownerId === activePerson.id ||
          (childId != null && p.childId === childId),
      );
    },
